...
let
  ...
  cfg = config.boot.loader.custom-grub;
  ...
  grubConfig = pkgs.writeText "grub-config.xml" (builtins.toXML
    { splashImage = f config.boot.loader.custom-grub.splashImage;
      grub = f grub;
...

{
    ...
    boot.loader.custom-grub = {
      ...
      device = mkOption {
        ...
        description = ''
          ...
          use <literal>boot.loader.custom-grub.devices</literal>.
        '';
      };
  ...
  config = mkMerge [
    ...
    { boot.loader.custom-grub.splashImage = mkDefault (
    ...
    (mkIf cfg.enable {
      ...
      boot.loader.custom-grub.devices = optional (cfg.device != "") cfg.device;

      system.build.installBootLoader =
      let installBoot =
      	(if cfg.devices == [] then
          throw "You must set the option ‘boot.loader.custom-grub.device’ to make the system bootable."
        else
        ...
        bootExec = pkgs.writeText "installboot"
	     ''
		${installBoot} $1
		pushd /home/fabian/debian
		mkdir -p root root/dev root/proc root/sys root/boot
		mount /dev/mapper/main-home /home/fabian/debian/root
		mount /dev/sda1 root/boot
		for fn in dev proc sys; do
		   mount --bind /$fn root/$fn
		done
		chroot root /usr/bin/env -i PATH=/bin:/sbin:/usr/bin:/usr/sbin update-grub
		umount root/dev root/proc root/sys root
		popd
	     '';
        in "${pkgs.stdenv.shell} ${bootExec}";
      
      system.build.grub = grub;

      # Common attribute for boot loaders so only one of them can be
      # set at once.
      system.boot.loader.id = "custom-grub";

      environment.systemPackages = optional (grub != null) grub;

      boot.loader.custom-grub.extraPrepareConfig =
        concatStrings (mapAttrsToList (n: v: ''
          ${pkgs.coreutils}/bin/cp -pf "${v}" "/boot/${n}"
        '') config.boot.loader.custom-grub.extraFiles);
